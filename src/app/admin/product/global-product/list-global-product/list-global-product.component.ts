...
@Component({
  selector: "app-list-global-product",
  templateUrl: "./list-global-product.component.html",
  styleUrls: [
    "../../../../scss/list.style.scss",
    "./list-global-product.component.scss",
  ],
})
export class ListGlobalProductComponent implements OnInit, OnDestroy {
  ...
  colData: ColData[] = [];
  globalProducts: ProductCategory[] = [];
  productId: number;
  ...
  ngOnInit(): void {
    ...
    this.subs.add(
      this.translate.onLangChange.subscribe(({ lang }) => {
        ...
        this.setColData(this.currentLang);
        this.setGridData(this.globalProducts);
      }),
  ...
  setGridData(data: ProductCategory[]) {
    this.gridData = data.map((product: ProductCategory) => {
      return {
        id: product.id,
        ...
        unit:
          this.currentLang === "en"
            ? this.units.find((unit) => unit.id === product.measurementUnitId)
                ?.enName ?? ""
            : this.units.find((unit) => unit.id === product.measurementUnitId)
                ?.localeName ?? "",
        country:
  ...
  getProducts(searchObj?: ProductSearch) {
    ...
    this.subs.add(
      this.productService
        .getProducts(
          { ...removeNullProps(searchObj), global: true },
          this.currentPage - 1,
          this.pageSize
        )
        .subscribe(
          (products: BaseResponse<ProductCategory>) => {
            if (products.content?.length > 0) {
              ...
              this.globalProducts = products.content;
              this.setGridData(this.globalProducts);
            } else {
              this.globalProducts = [];
